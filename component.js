...
exports.handlers = {
  beforeParse: function(e) {
    if (path.extname(e.filename) === '.vue') {
      ...
      var script = e.source.match(new RegExp('<script>(.*?)</script>', 's')) || e.source.match(new RegExp('<script lang="ts">(.*?)</script>', 's')) || e.source.match(new RegExp('<script lang=\'ts\'>(.*?)</script>', 's'))
      e.source = script[1]
    }
  },
